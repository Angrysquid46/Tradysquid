...
def filters_reply() -> str:
    ...
    return "\n".join([
        ...
        "",
        "**SPY 0-1 DTE Expansion-Level** - a third, fully independent SPY",
        f"strategy ({'on' if traders.get('spy_expansion_level', False) else 'off'}). Six reference levels (prior day/",
        "week/month high-low) + 20/200 EMA and MACD histogram alignment across",
        "15m/30m/1h; unrelated to the strategies above.",
        f"Level tolerance: **${spy_scanner.SPY_EXPANSION_LEVEL_TOLERANCE:.2f}**",
        f"Delta range: **{spy_scanner.SPY_EXPANSION_DELTA_MIN:.2f}–{spy_scanner.SPY_EXPANSION_DELTA_MAX:.2f}**",
        f"Stop / target: **-{spy_scanner.SPY_EXPANSION_STOP_PCT * 100:.0f}% / +{spy_scanner.SPY_EXPANSION_TARGET_PCT * 100:.0f}%**",
        f"Floor lock: triggers at **+{spy_scanner.SPY_EXPANSION_FLOOR_TRIGGER_PCT:.0f}%**, raises stop to **{spy_scanner.SPY_EXPANSION_FLOOR_PCT:.0f}%**",
        f"Max contract ask / position risk: **${spy_scanner.SPY_EXPANSION_MAX_CONTRACT_ASK:.2f} / ${spy_scanner.SPY_EXPANSION_MAX_RISK_PER_TRADE:.0f}**",
        "",
