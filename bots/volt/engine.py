"""Private causal implementation of the selected sensitive SURGE challenger."""
...
class Volt:
    ...
    def signal(bars):
        ...
        score=min(1.,.51*efficiency+.35*min(min(2.,recent/prior)/1.5,1)+.14*min(abs(net)/.8,1))
        flips=sum((c[i]-c[i-1])*(c[i-1]-c[i-2])<0 for i in range(2,len(c)))
        if efficiency<.15 or flips>=11:return None,score,"CHOP"
    ...
    def decide(self,as_of,bankroll,market,options,bars):
        ...
        return Decision("ENTER","SENSITIVE_THREE_MINUTE_IMPULSE",side,str(contract["option_symbol"]),ask,qty,score)
