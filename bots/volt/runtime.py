...
from .engine import MULTIPLIER,Decision,Position,Volt
BOT="VOLT";STATE_DIR=Path(__file__).resolve().parents[2]/"state"/"volt";TELEMETRY=STATE_DIR/"live-decisions.jsonl";POSITION_STATE=STATE_DIR/"position-state.json"
class VoltRuntime:
    def __init__(self,engine=None,market_view=None,telemetry_path=None):self.engine=engine or Volt();self.market_view=market_view or backtest_lab.MarketView("SPY");self.telemetry_path=telemetry_path or TELEMETRY
    def recover(self,c):
    ...
    def evaluate(self,as_of:datetime,c:sqlite3.Connection)->Decision:
        ...
        if d.action=="ENTER":
            trade_id=f"volt-{uuid.uuid4()}";scoreboard.record_trade_open(c,trade_id=trade_id,bot=BOT,generation=self.engine.generation,opened_at=as_of.isoformat(),side=str(d.side),contract_symbol=str(d.contract_symbol),entry_price=float(d.price),contracts=d.contracts,entry_bankroll=bankroll)
            ...
            scoreboard.record_generation_event(c,bot=BOT,generation=self.engine.generation,event="BUSTED",detail=d.reason,minimum_qualifying_cost=d.minimum_qualifying_cost or bankroll+.02)
