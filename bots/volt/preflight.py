...
def run(session_date:date,require_clean_start=True):
    ...
    checks=[Check("deployed-main",bool(deployed and head.startswith(deployed) and state.get("last_update_status") in {"DEPLOYED","UP_TO_DATE"}),f"head={head[:12]} deployed={deployed[:12]}"),Check("single-instance",port_free(),"VOLT port 8896 free")]
    c=scoreboard.connect_db()
    try:g=scoreboard.current_generation(c,"VOLT");b=scoreboard.current_bankroll(c,"VOLT");t=scoreboard.trade_count(c,"VOLT");flat=scoreboard.current_position_status(c,"VOLT") is None
    finally:c.close()
    checks.append(Check("official-state",(abs(b-1000)<.01 and t==0 and flat) if require_clean_start else g>=1,f"generation={g} bankroll={b:.2f} trades={t} flat={flat}"));return checks
