...
def test_setup_reports_exact_live_preflight_failure_and_preserves_warning() -> None:
    ...
    assert '"Live preflight failed: category={0}; check={1}; error={2}" -f' in text
    assert "$LiveSummary.category" in text
    assert "$LiveSummary.failed_check" in text
    assert "$LiveSummary.error" in text
