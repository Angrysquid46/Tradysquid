...
def test_setup_entry_guards_parse_time_failures() -> None:
    ...
    assert "setup-entry-exit-code.txt" in text
...
def test_wrapper_helper_waits_for_real_child_exit_and_captures_streams(
    ...
    exit_code = state / "setup-entry-exit-code.txt"
    ...
    entry.write_text(
        ...
        f"'0' | Set-Content -LiteralPath '{exit_code}' -Encoding ASCII\n"
    ...
    assert Path(receipt["EntryExitCodePath"]) == exit_code
...
def test_verified_handoff_installs_all_five_canonical_names(tmp_path: Path) -> None:
    ...
    assert destination.read_text(encoding="utf-8-sig").splitlines() == _canonical_handoff_text().splitlines()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            _canonical_handoff_text().replace("TRADIER_ACCESS_TOKEN=test-tradier-token", "TRADIER_ACCESS_TOKEN="),
            "blank value",
        ),
    ],
)
...
def test_wrapper_helper_restores_original_env_exactly(tmp_path: Path) -> None:
    ...
    command = (
        f". '{HELPERS}'; "
        f"Restore-FileExact -BackupPath '{backup}' -DestinationPath '{destination}'"
    )
    subprocess.run(
        [powershell, "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", command],
        cwd=tmp_path,
