...
def _row(hours_ago: float, **overrides) -> dict[str, str]:
    ...
    entered = spy_scanner.now_ct() - timedelta(hours=hours_ago)
...
def test_a_fresh_position_never_triggers_this_regardless_of_theta_burden():
    ...
    triggered, note = spy_scanner.check_time_efficiency_exit(
        row, theta=-0.10, mark=1.00, pnl_pct=0.0, timestamp=spy_scanner.now_ct()
    )
...
def test_a_stuck_position_with_high_theta_burden_after_real_time_triggers():
    ...
    triggered, note = spy_scanner.check_time_efficiency_exit(
        row, theta=-0.08, mark=1.00, pnl_pct=1.0, timestamp=spy_scanner.now_ct()
    )
...
def test_a_stuck_position_with_low_theta_burden_does_not_trigger():
    row = _row(hours_ago=24.0)
    triggered, note = spy_scanner.check_time_efficiency_exit(
        row, theta=-0.01, mark=1.00, pnl_pct=1.0, timestamp=spy_scanner.now_ct()
    )
    assert triggered is False
...
def test_a_position_clearly_heading_toward_a_real_loss_is_not_stuck():
    ...
    row = _row(hours_ago=24.0)
    triggered, note = spy_scanner.check_time_efficiency_exit(
        row, theta=-0.20, mark=1.00, pnl_pct=-25.0, timestamp=spy_scanner.now_ct()
    )
    assert triggered is False
...
def test_missing_theta_never_triggers_or_crashes():
    row = _row(hours_ago=24.0)
    triggered, note = spy_scanner.check_time_efficiency_exit(
        row, theta=None, mark=1.00, pnl_pct=1.0, timestamp=spy_scanner.now_ct()
    )
    assert triggered is False
...
def test_missing_entry_timestamp_never_triggers_or_crashes():
    ...
    row["timestamp"] = ""
    triggered, note = spy_scanner.check_time_efficiency_exit(
        row, theta=-0.20, mark=1.00, pnl_pct=1.0, timestamp=spy_scanner.now_ct()
    )
    assert triggered is False
...
def test_zero_or_negative_mark_never_triggers_or_crashes_on_division():
    row = _row(hours_ago=24.0)
    triggered, note = spy_scanner.check_time_efficiency_exit(
        row, theta=-0.20, mark=0.0, pnl_pct=1.0, timestamp=spy_scanner.now_ct()
    )
    assert triggered is False
...
def test_end_to_end_through_evaluate_open_row_actually_fires():
    ...
    entered = spy_scanner.now_ct() - spy_scanner.timedelta(hours=30)
    row = {field: "" for field in spy_scanner.LOG_HEADER}
    row.update({
