...
def trade_title(row: dict[str, str]) -> str:
    ...
    if play_type == "SPREAD":
        ...
        direction = "PUT CREDIT" if kind == "PUT" else "CALL CREDIT"
        return f"{trade_id} | F {direction} {fmt_strike(sell_strike)}/{fmt_strike(buy_strike)} | {expiration}"
    return f"{trade_id} | F {play_type} {kind} {row.get('strike', '')} | {expiration}"
...
class DiscordTracker:
    ...
    def send_thread(self, thread_id: str, embed: dict[str, Any]) -> None:
        ...
        try:
            ...
            if thread_id:
                ...
                discord.send_channel(
                    "entry",
                    content=f"🧾 **{row.get('trade_id')}** imported into paper tracking. {link}",
                )
        except DiscordError as exc:
            print(f"Could not backfill Discord thread for {row.get('trade_id')}: {exc}", file=sys.stderr)
    return created
...
def post_material_update(row: dict[str, str], evaluation: dict[str, Any], discord: DiscordTracker, timestamp: datetime) -> None:
    if not discord.ready or not row.get("discord_thread_id") or not should_post_update(row, evaluation, timestamp):
        return
    embed = update_embed(row, evaluation)
    discord.send_thread(row["discord_thread_id"], embed)
    discord.send_channel(
        "updates",
        content=f"**{row.get('trade_id')}** • {evaluation.get('signal')} • {fmt_pct(as_float(evaluation.get('pl_pct')))} • {thread_link(row['discord_thread_id'])}",
    )
    status = "HOLDING"
...
def post_close(row: dict[str, str], evaluation: dict[str, Any], discord: DiscordTracker) -> None:
    ...
    embed = close_embed(row, evaluation)
    if thread_id:
        discord.send_thread(thread_id, embed)
        discord.set_thread_status(thread_id, row["outcome"], archive=True)
    link = thread_link(thread_id)
    summary = f"**{row.get('trade_id')}** • {row.get('outcome')} • {fmt_pct(as_float(evaluation.get('pl_pct')))}"
    if link:
        summary += f" • {link}"
    discord.send_channel("exit", content=summary, embed=embed)
    result_channel = {"WIN": "wins", "LOSS": "losses", "SCRATCH": "scratches"}.get(row["outcome"])
    if result_channel:
        discord.send_channel(result_channel, content=summary, embed=embed)
...
def post_new_trade(row: dict[str, str], discord: DiscordTracker) -> None:
    ...
    thread_id = discord.create_trade_thread(row, "OPEN")
    link = thread_link(thread_id)
    discord.send_channel("qualified", content=f"✅ **{trade_title(row)}**\n{link}", embed=setup_embed(row))
    discord.send_channel("entry", content=f"📥 Paper entry tracked: **{row.get('trade_id')}** • {link}")
