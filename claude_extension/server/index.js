...
function handle(message) {
  ...
  if (message.method === "initialize") {
    respond(message.id, {
      ...
      instructions: "This is the private maintenance connector for Cody's Tradysquid trading-information system: its SPY 0DTE options scanner, Tradier market data, Discord TradeBot, TradingView charts/webhooks, paper-trade lifecycle, GitHub Actions, and sanitized learning archive. When Cody mentions Tradysquid, the scanner, the bot, Discord, SPY, options, 0DTE, charts, news, schedules, or upgrades, treat this repository as the project he means and call coordination_status before answering or working. Read CLAUDE.md and AI_COLLABORATION.md when deeper project context is needed. Acquire begin_update before every write. Preserve unrelated changes. Run tests, publish explicit files, then finish_update. Never expose secrets, bypass the shared lock, force-push, or execute brokerage trades.",
    });
    return;
  }
