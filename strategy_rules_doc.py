...
def build() -> str:
    ...
    lines += [
        ...
        "",
        "- The 10 ratchet variants and `SPY_EXPANSION_LEVEL` - retired, and "
        "their code was deleted outright on 2026-08-19. Nothing is left to "
        "re-enable.",
        "- `SPY_0DTE_1M` / `SPY_0DTE_5M`, `SPY_GAP_CONT_25`, `SPY_GAP_CONT_100`, "
        "`SPY_SWEEP_5` - retired play types. Closed rows survive in the trade "
        "log and are filtered out of all reporting by "
        "`performance_reconciliation._RETIRED_ROUTE_PLAY_TYPES`, which is why "
        "those strings still appear in the code.",
        "- `spy_0dte_exit_signal` / `scan_spy_0dte_candidates` remain in "
        "`spy_scanner` despite the play types being retired: `evolve_bot` "
        "imports both, and it is kept as a separate controlled test.",
