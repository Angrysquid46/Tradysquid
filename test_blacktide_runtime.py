...
def test_runtime_records_one_immutable_round_trip(tmp_path, monkeypatch):
    ...
    runtime = BlacktideRuntime(market_view=view, evolution=EvolutionLoop(tmp_path / "outcomes.jsonl"),
                               position_state_path=tmp_path / "position-state.json")
...
def test_restart_recovers_open_position_and_cannot_double_open(tmp_path, monkeypatch):
    ...
    first = BlacktideRuntime(market_view=view, evolution=evolution, position_state_path=state_path)
    assert first.evaluate(NOW, db).action == "ENTER"
    expected = (first.engine.position.entry_state, first.engine.position.entry_family)
    restarted = BlacktideRuntime(market_view=view, evolution=evolution, position_state_path=state_path)
    result = restarted.evaluate(NOW + timedelta(minutes=1), db)
    assert result.action == "NO_ACTION"
    assert restarted.engine.position is not None
    assert (restarted.engine.position.entry_state, restarted.engine.position.entry_family) == expected
...
def test_audit_recovers_from_legacy_naive_timestamp(tmp_path, monkeypatch):
    monkeypatch.setattr(blacktide_runtime, "STATE_DIR", tmp_path)
    monkeypatch.setattr(blacktide_runtime, "DECISION_LOG_PATH", tmp_path / "decision-audit.jsonl")
    monkeypatch.setattr(blacktide_runtime, "DECISION_STATE_PATH", tmp_path / "decision-audit-state.json")
    (tmp_path / "decision-audit-state.json").write_text(
        '{"signature":"old","observed_at":"2026-08-25T10:01:00"}', encoding="utf-8"
    )
    BlacktideRuntime._record_decision(
        Decision("NO_ACTION", "current"), NOW.replace(tzinfo=timezone.utc)
    )
    assert '"reason": "current"' in (tmp_path / "decision-audit.jsonl").read_text(encoding="utf-8")
