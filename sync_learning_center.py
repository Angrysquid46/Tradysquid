"""Synchronize the long-form options curriculum into Discord topic channels."""
...
def split_long_block(block: str, limit: int) -> list[str]:
    """Split one oversized paragraph without exceeding Discord's message limit."""
...
def chunk_markdown(text: str, limit: int = CHUNK_TARGET) -> list[str]:
    """Chunk Markdown at paragraph boundaries while preserving readable lessons."""
    ...
    for paragraph in paragraphs:
        paragraph_parts = (
            ...
            else split_long_block(paragraph, limit)
        )
    ...
    if current:
        chunks.append(current)
    if not chunks:
        raise RuntimeError("Curriculum lesson produced no Discord messages.")
    return chunks
...
def expected_messages(channel: str, text: str) -> list[str]:
    chunks = chunk_markdown(text)
    messages = [
        f"{lesson_marker(channel, index, len(chunks))}\n\n{chunk}"
        for index, chunk in enumerate(chunks, start=1)
    ]
    too_long = [len(message) for message in messages if len(message) > MAX_DISCORD_CONTENT]
    if too_long:
        raise RuntimeError(
            f"{channel} produced oversized Discord messages: {too_long}"
        )
    return messages
...
def synchronize_curriculum() -> dict[str, int]:
    ...
    if not tracker.enabled:
        raise RuntimeError(
            "DISCORD_BOT_TOKEN and DISCORD_GUILD_ID are required."
        )
...
def validate_curriculum() -> dict[str, int]:
    ...
    print(
        ...
        f"{sum(counts.values())} Discord message parts."
    )
    return counts
