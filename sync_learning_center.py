...
def _marker_from_message(message: dict[str, Any], channel: str) -> str:
    ...
    for line in discord_transport.message_search_text(message).splitlines():
...
def _is_old_learning_message(message: dict[str, Any], channel: str) -> bool:
    if not _is_bot_message(message):
        return False
    text = discord_transport.message_search_text(message).strip()
...
def synchronize_curriculum(
    ...
    tracker = tracker or discord_transport.DiscordTracker(
