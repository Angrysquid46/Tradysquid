...
def test_spread_ok_accepts_tight_spread():
    assert spread_ok({"bid": 3.95, "ask": 4.0}, PARAMS) is True


def test_spread_ok_rejects_wide_spread():
    assert spread_ok({"bid": 1.0, "ask": 4.0}, PARAMS) is False


def test_spread_ok_rejects_bad_quotes():
    assert spread_ok({"bid": 0, "ask": 4.0}, PARAMS) is False
    assert spread_ok({"bid": 4.0, "ask": 3.9}, PARAMS) is False
    assert spread_ok({"bid": None, "ask": 4.0}, PARAMS) is False


def test_execution_assumptions_disclose_no_hindsight_or_mid_fills():
    assumptions = build_execution_assumptions(PARAMS)
