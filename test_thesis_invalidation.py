...
def _overbought_fade_closes() -> list[float]:
    # Steady climb pins RSI at the ceiling, then a 3-day fade rolls it
    # over - the confirmed-reversal shape swing's BEARISH regime needs.
    closes = [100.0]
    for _ in range(39):
        closes.append(round(closes[-1] * 1.015, 2))
    for _ in range(3):
        closes.append(round(closes[-1] * 0.97, 2))
    return closes
...
def test_a_call_held_through_a_genuine_reversal_to_bearish_is_flagged():
    with _PrimedCache(_overbought_fade_closes()):
...
def test_a_ticker_outside_the_validated_swing_set_never_flags_a_reversal():
    ...
    with _PrimedCache(_overbought_fade_closes(), ticker="F"):
