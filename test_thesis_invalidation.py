...
in spy_scanner.py) and reacts to a confirmed pullback-in-trend (buy the dip
...
class _PrimedCache:
    ...
    def __enter__(self):
        spy_scanner.DAILY_SNAPSHOT_CACHE[self.ticker] = (spy_scanner.time.monotonic(), _daily_history(self.closes))
        spy_scanner.INTRADAY_SNAPSHOT_CACHE[self.ticker] = (spy_scanner.time.monotonic(), [])

    def __exit__(self, *exc):
        spy_scanner.DAILY_SNAPSHOT_CACHE.pop(self.ticker, None)
        spy_scanner.INTRADAY_SNAPSHOT_CACHE.pop(self.ticker, None)


def test_spreads_are_never_evaluated_for_thesis_invalidation():
    row = _row(play_type="SPREAD", call_or_put="put")
    invalidated, note = spy_scanner.check_thesis_invalidation(row, spy_scanner.now_ct())
...
def test_cache_priming_does_not_leak_between_tests():
    ...
    with _PrimedCache(_uptrend_closes()):
        assert "SOFI" in spy_scanner.DAILY_SNAPSHOT_CACHE
    assert "SOFI" not in spy_scanner.DAILY_SNAPSHOT_CACHE
    assert "SOFI" not in spy_scanner.INTRADAY_SNAPSHOT_CACHE
