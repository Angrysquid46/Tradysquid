...
def _with_temp_iv_history():
    ...
    class _Swap:
        def __enter__(self):
            self.original = spy_scanner.IV_HISTORY_PATH
            self.tmp = tempfile.TemporaryDirectory()
            spy_scanner.IV_HISTORY_PATH = Path(self.tmp.name) / "iv-history.json"
            return spy_scanner.IV_HISTORY_PATH

        def __exit__(self, *exc):
            spy_scanner.IV_HISTORY_PATH = self.original
            self.tmp.cleanup()
...
def test_realized_volatility_is_none_with_too_little_history():
    assert spy_scanner.realized_volatility([100.0, 101.0, 99.0]) is None


def test_realized_volatility_is_near_zero_for_a_dead_flat_tape():
    flat = [100.0] * 25
    rv = spy_scanner.realized_volatility(flat)
    assert rv is not None
    assert rv < 0.01


def test_realized_volatility_is_meaningfully_higher_for_a_choppier_tape():
    calm = spy_scanner.realized_volatility(_calm_midrange_closes())
    choppy = [100.0] * 40 + [
        round(100 + 8 * ((-1) ** i), 2) for i in range(20)
    ]
    volatile = spy_scanner.realized_volatility(choppy)
...
def test_pooled_iv_rank_is_none_when_the_whole_pool_is_thin():
    with _with_temp_iv_history():
        ...
        for i, ticker in enumerate(["AAA", "BBB", "CCC"]):
            spy_scanner.record_iv_snapshot(ticker, 0.30, "2026-01-15")
        rank, samples = spy_scanner.pooled_iv_rank(0.30, "2026-01-15")
        assert rank is None
        assert samples == 3
...
def test_pooled_iv_rank_gives_a_brand_new_ticker_a_real_read_on_day_one():
    with _with_temp_iv_history():
        ...
        for i in range(25):
            spy_scanner.record_iv_snapshot(f"T{i}", 0.25, "2026-01-15")
        rank, samples = spy_scanner.pooled_iv_rank(0.50, "2026-01-15")
...
def test_pooled_iv_rank_ignores_readings_older_than_the_lookback_window():
    with _with_temp_iv_history():
        for i in range(25):
            spy_scanner.record_iv_snapshot(f"T{i}", 0.90, "2025-01-01")  # ancient, way outside window
        rank, samples = spy_scanner.pooled_iv_rank(0.30, "2026-01-15", lookback_days=20)
        assert samples == 0
        assert rank is None
...
def test_spread_context_uses_pooled_rank_when_tickers_own_history_is_thin():
    ...
    spot = closes[-1]
    realized = spy_scanner.realized_volatility(closes)
...
def test_iv_rank_is_none_before_enough_history_exists():
    with _with_temp_iv_history():
        for day in range(5):  # well under IV_HISTORY_MIN_SAMPLES
            spy_scanner.record_iv_snapshot("VALE", 0.40, f"2026-01-{day + 1:02d}")
        assert spy_scanner.iv_rank("VALE") is None
...
def test_iv_rank_reflects_where_todays_reading_sits_in_its_own_history():
    with _with_temp_iv_history():
        ...
        for day in range(20):
            spy_scanner.record_iv_snapshot("VALE", 0.30, f"2026-01-{day + 1:02d}")
        spy_scanner.record_iv_snapshot("VALE", 0.60, "2026-01-21")
        rank = spy_scanner.iv_rank("VALE")
        assert rank is not None
        assert rank >= 95.0
...
def test_same_day_recorded_twice_updates_instead_of_duplicating():
    with _with_temp_iv_history():
        spy_scanner.record_iv_snapshot("VALE", 0.30, "2026-01-01")
        spy_scanner.record_iv_snapshot("VALE", 0.35, "2026-01-01")
        history = spy_scanner._load_iv_history()
...
def test_bull_put_spread_strike_must_sit_below_real_support():
    ...
    candidates = spy_scanner.scan_credit_spreads(chain, "put", "2026-09-18", context)
...
def test_spread_exit_takes_profit_at_half_of_max_like_before():
    signal, note = spy_scanner.spread_exit_signal(
        entry_credit=0.30,
...
def test_spread_exit_holds_in_the_normal_middle_ground():
    signal, _ = spy_scanner.spread_exit_signal(
        entry_credit=0.30,
        cost_to_close=0.25,
