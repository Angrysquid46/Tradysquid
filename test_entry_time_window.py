...
def _ct(hour: int, minute: int) -> datetime:
    ...
    return datetime(2026, 8, 5, hour, minute, tzinfo=ford_scan.now_ct().tzinfo)


def test_right_at_the_open_is_blocked():
    reason = ford_scan.entry_window_blocked(_ct(8, 30))
    assert reason != ""
    assert "first" in reason


def test_just_before_the_opening_exclusion_ends_is_still_blocked():
    reason = ford_scan.entry_window_blocked(_ct(8, 44))  # 14 min in, exclusion is 15
    assert reason != ""


def test_right_after_the_opening_exclusion_ends_is_allowed():
    reason = ford_scan.entry_window_blocked(_ct(8, 46))  # 16 min in
    assert reason == ""


def test_midmorning_well_clear_of_both_windows_is_allowed():
    reason = ford_scan.entry_window_blocked(_ct(9, 30))
    assert reason == ""


def test_inside_the_midday_lull_is_blocked():
    reason = ford_scan.entry_window_blocked(_ct(11, 0))
    assert reason != ""
    assert "lull" in reason


def test_afternoon_after_the_lull_is_allowed_again():
    reason = ford_scan.entry_window_blocked(_ct(13, 30))
    assert reason == ""
...
def test_outside_market_hours_is_not_specifically_blocked_by_this_check():
    ...
    reason = ford_scan.entry_window_blocked(_ct(20, 0))
    assert reason == ""


def test_a_weekend_is_not_specifically_blocked_by_this_check():
    saturday = datetime(2026, 8, 8, 10, 0, tzinfo=ford_scan.now_ct().tzinfo)
    reason = ford_scan.entry_window_blocked(saturday)
    assert reason == ""
