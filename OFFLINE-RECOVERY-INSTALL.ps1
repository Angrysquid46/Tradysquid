...
Write-Host 'Tradysquid final offline deployment v4' -ForegroundColor Cyan
...
$fetchCommand = 'git -C "{0}" fetch "{1}" "refs/remotes/origin/main:{2}" 2>&1' -f $Repository, $Bundle, $TargetRef
$fetchOutput = & cmd.exe /d /s /c $fetchCommand
if ($LASTEXITCODE -ne 0) {
    throw "Bundle import failed. Nothing was stopped or changed.`n$($fetchOutput -join "`n")"
}
